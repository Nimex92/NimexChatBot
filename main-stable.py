...
AGENDA_FILE = os.getenv("AGENDA_FILE", "agenda.json")
...
def formatear_nombre(asistente):
    if isinstance(asistente, dict):
        if asistente.get("nombre"):
            string = asistente["nombre"] + " (@" + asistente.get("username", "") + ")"
            return string
        elif asistente.get("username"):
            return f"@{asistente['username']}"
    return "Desconocido"
...
async def manejar_mensaje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not texto:  # si no hay texto (sticker, foto, etc.), ignoramos
        return
...
async def saludar_nuevo_miembro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    for nuevo in update.message.new_chat_members:
        nombre = nuevo.first_name or "amigo"
        user_id = nuevo.id

        saludo = (
            f"🙌 ¡Qué bueno tenerte por aquí, [{nombre}](tg://user?id={user_id})!\n"
            "Yo soy el bot del grupo, tu compa para planear quedadas, apuntarte a eventos o lo que surja 🧠🎯\n"
            "Escríbeme cuando quieras o prueba /agenda para empezar 🚀"
        )
...
def main():
    ...
    app.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, saludar_nuevo_miembro))
